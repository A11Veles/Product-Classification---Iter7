...
#     tdf_gpc_class = DataFrame.from_table("gpc_orig", schema_name="demo_user")[[ "ClassTitle", "ClassDefinition"]]
#     gpc_class_df = tdf_gpc_class.to_pandas()
#     gpc_class_df["class_text"] = gpc_class_df["ClassTitle"].fillna("") + ". " + gpc_class_df["ClassDefinition"].fillna("")
...
#     pred_df = icf_tdcb_predict(gpc_class_df, df, k=3)
#     print(pred_df.head())
...
# 2. Main function
# ------------------------
def icf_tdcb_predict(
    gpc_df, products_df, k=3,
    class_id_col="ClassTitle",          # use ClassTitle as ID
    class_name_col="ClassTitle",        # also use ClassTitle as name
    class_text_col="class_text",
    product_id_col="product_id",        # change if your products table has a different id col
...
if __name__ == "__main__":
    ...
    # Load GPC (Class level)
    tdf_gpc_class = DataFrame.from_table("gpc_orig", schema_name="demo_user")[["ClassTitle", "ClassDefinition"]]
    gpc_class_df = tdf_gpc_class.to_pandas()
    gpc_class_df["class_text"] = (
        gpc_class_df["ClassTitle"].fillna("") + ". " + gpc_class_df["ClassDefinition"].fillna("")
    )
